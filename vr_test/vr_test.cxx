...
void vr_test::draw(cgv::render::context& ctx)
{
	...
	/*
	if (!pc_test.empty()) {
		//std::cout << "pc_test not empty" << std::endl;
		auto& sr_test = cgv::render::ref_sphere_renderer(ctx); 
		srs.radius = 0.001f;
		sr_test.set_render_style(srs);						 
		sr_test.set_position_array(ctx, pc_test);			  
		sr_test.render(ctx, 0, pc_test.size());				  
	}*/

	//render normal vectors
	std::vector<vec3> arrow_test;
	arrow_test.push_back((0, 0, 0));
	std::vector<vec3> arrow_test_end;
	arrow_test_end.push_back((1, 1, 1));

	
	if (!arrow_test.empty()) {
		// std::cout << "arrow_test not empty" << std::endl;
		auto& ar_test = cgv::render::ref_arrow_renderer(ctx);
		//ars?= 0.001f;
		ar_test.set_render_style(ars);
		ar_test.set_position_array(ctx, arrow_test);
		ar_test.set_end_point_array(ctx, arrow_test_end);
		ar_test.render(ctx, 0, arrow_test.size());
	}
...
bool vr_test::readObjFile()
{
	const std::string& objfilename = "E:/cgv_develop/cgv/vr_test/data/stanford-bunny.obj";
	std::ifstream file(objfilename);
	if (!file.is_open()) {
		std::cerr << "Unable to open file: " << objfilename << std::endl;
		return false;
	}
	...
	while (std::getline(file, line)) {
		...
		if (lineHeader == "v") {
			...
			pc_test.push_back(vertex);
		}
	}
	return true;
}

bool vr_test::calculate_ann()
{
	/*//create a matrix to store the points
	cgv::math::mat<double> points(pc_test.size(), 3);

	// from std::vector<vec3> to mat<double>
	for (size_t i = 0; i < pc_test.size(); ++i) {
		points(i, 0) = pc_test[i].x();
		points(i, 1) = pc_test[i].y();
		points(i, 2) = pc_test[i].z();
	}
	std::cout << "transform finish，data size：" << points.size() << std::endl;

	cgv::math::ann cloud_ANN; // create ann instance;
	cloud_ANN.setup(points);  // set ann with transformed points
	std::cout << "ANN set finish" << std::endl;

	int k = 5;					  // set k value
	cgv::math::vec<double> queryPoint; // define the query point
	// set the query point coordinates
	queryPoint.x() = pc_test[0].x();
	queryPoint.y() = pc_test[0].y();
	queryPoint.z() = pc_test[0].z();

	int* indices = new int[k];		  // indice arrays
	double* sqrdists = new double[k]; // distance arrays

	// do it
	cloud_ANN.query_knn(queryPoint, k, indices, sqrdists);

	// use indices and sqrdists here
	std::cout << "query completed. result：" << std::endl;
	for (int i = 0; i < k; ++i) {
		std::cout << "nearest " << i << ": indice = " << indices[i] << ", sqrtdis = " << sqrdists[i] << std::endl;
	}

	delete[] indices; // clear
	delete[] sqrdists;*/
	
	// allocate point data
	ANNpointArray annPoints = annAllocPts(pc_test.size(), 3); // 3表示空间的维度

	// transform to ANN type
	for (size_t i = 0; i < pc_test.size(); ++i) {
		annPoints[i][0] = pc_test[i].x();
		annPoints[i][1] = pc_test[i].y();
		annPoints[i][2] = pc_test[i].z();
	}

	// create kd-tree
	ANNkd_tree* kdTree = new ANNkd_tree(annPoints,	   // point data
										pc_test.size(), // point number
										3);			   // dimension

	// query point set
	vec3 query = (pc_test[0].x(),pc_test[0].y(),pc_test[0].z());
	ANNpoint queryPt = annAllocPt(3); // allocate point
	queryPt[0] = query.x();
	queryPt[1] = query.y();
	queryPt[2] = query.z();

	// result space
	ANNidxArray nnIdx = new ANNidx[1];	 //index
	ANNdistArray dists = new ANNdist[1]; // distance
	...
	std::cout << "nnidx:" << *nnIdx << "dist:" << *dists << std::endl;
